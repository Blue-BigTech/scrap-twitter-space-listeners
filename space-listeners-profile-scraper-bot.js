  ...
  page.on("request", async (request) => {
    if (request.url().includes("https://api.twitter.com/graphql/t")) {
      ...
      try {
        ...
        const favourites_count =
          parsed.data.user.result.legacy.favourites_count;
        const profile_image_url =
          parsed.data.user.result.legacy.profile_image_url_https;
        ...
        if (professional_type) {
          professional_category =
            parsed.data.user.result.professional.category[0];
          ...
          professional_type =
            parsed.data.user.result.professional.professional_type;
        } else {
